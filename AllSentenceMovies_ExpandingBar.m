function AllSentenceMovies_ExpandingBar(outDir, xlsFilename)
%% AllSentenceMovies_ExpandingBar(outDir, xlsFilename)
...
% For each sentence, a movie is created in which a bar expands through the 
...
barBGColour   	= 'w'; % (of the form 'r' or [.7 .7 .7])
barFillColour	= 'r'; % (of the form 'r' or [.7 .7 .7]) 
...
for a = 1 : length(numberToPlot);
    ...
    fprintf('\nSentence: "%s"\n', sentence);
    createSentenceMovie_ExpandingBar(wordArray, numberToPlot(a), colours, word_spacing, ...
        bg_colour, barInterval, barBGColour, barFillColour, ...
        movieFilename, fps, ceil(numSentFrames*1.2), nFrPause);
    close all;
end
